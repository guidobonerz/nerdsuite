...
public class Binary2SourceConverter
{
	...
	public static StringBuilder convertBinaryToText(File file, int sourceFormat, int valueOutputFormat, boolean skipLocation, boolean writeComment)
	{
		...
		byte byteArray[] = BinaryFileReader.readFile(file);
		int index = 0;
		int width = 1;
		int height = 8;
		int step = 1;
		if ((sourceFormat & CHARSET + CHARSET_DOUBLE_X + CHARSET_DOUBLE_Y) == CHARSET + CHARSET_DOUBLE_X + CHARSET_DOUBLE_Y)
		{
	...
	{
		StringBuilder sb = convertBinaryToText(new File("/Users/drazil/Downloads/gi-joe_major_bludd.spr"), SPRITE + MULTICOLOR, AS_HEX_VALUE, false, true);
