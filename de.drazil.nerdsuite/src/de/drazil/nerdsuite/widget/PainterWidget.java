...
public class PainterWidget extends BaseImagingWidget {
	...
	protected void paintControl(GC gc, RedrawMode redrawMode, boolean paintPixelGrid, boolean paintSeparator,
		...
		paintPixelCursor(gc, conf.cursorMode);
	...
	private void paintPixelCursor(GC gc, CursorMode cursorMode) {
		if (computeCursorIndex(cursorX, cursorY) < conf.tileSize
			...
			if (cursorMode == CursorMode.SelectRectangle) {
				...
				gc.setXORMode(true);
				if (!isDragging) {
					gc.drawLine((x * pixelWidth) - 10, y * pixelHeight, (x * pixelWidth) + 10, y * pixelHeight);
					gc.drawLine(x * pixelWidth, (y * pixelHeight) - 10, x * pixelWidth, (y * pixelHeight) + 10);
				} else {
					if (selectedPixelRangeX2 >= selectedPixelRangeX && selectedPixelRangeY2 >= selectedPixelRangeY) {
						gc.drawLine(selectedPixelRangeX * pixelWidth, selectedPixelRangeY * pixelHeight,
								(selectedPixelRangeX * pixelWidth) + 10, selectedPixelRangeY * pixelHeight);
						gc.drawLine(selectedPixelRangeX * pixelWidth, selectedPixelRangeY * pixelHeight,
								selectedPixelRangeX * pixelWidth, (selectedPixelRangeY * pixelHeight) + 10);
					} else if (selectedPixelRangeX2 >= selectedPixelRangeX
							&& selectedPixelRangeY2 < selectedPixelRangeY) {
						gc.drawLine(selectedPixelRangeX * pixelWidth, selectedPixelRangeY * pixelHeight + pixelHeight,
								(selectedPixelRangeX * pixelWidth) + 10,
								selectedPixelRangeY * pixelHeight + pixelHeight);
						gc.drawLine(selectedPixelRangeX * pixelWidth, selectedPixelRangeY * pixelHeight + pixelHeight,
								selectedPixelRangeX * pixelWidth,
								(selectedPixelRangeY * pixelHeight) - 10 + pixelHeight);
					} else if (selectedPixelRangeX2 < selectedPixelRangeX
							&& selectedPixelRangeY2 < selectedPixelRangeY) {
						gc.drawLine(selectedPixelRangeX * pixelWidth + pixelWidth,
								selectedPixelRangeY * pixelHeight + pixelHeight,
								(selectedPixelRangeX * pixelWidth) - 10 + pixelWidth,
								selectedPixelRangeY * pixelHeight + pixelHeight);
						gc.drawLine(selectedPixelRangeX * pixelWidth + pixelWidth,
								selectedPixelRangeY * pixelHeight + pixelHeight,
								selectedPixelRangeX * pixelWidth + pixelWidth,
								(selectedPixelRangeY * pixelHeight) - 10 + pixelHeight);
					} else if (selectedPixelRangeX2 < selectedPixelRangeX
							&& selectedPixelRangeY2 >= selectedPixelRangeY) {
						gc.drawLine(selectedPixelRangeX * pixelWidth + pixelWidth, selectedPixelRangeY * pixelHeight,
								(selectedPixelRangeX * pixelWidth) - 10 + pixelWidth,
								selectedPixelRangeY * pixelHeight);
						gc.drawLine(selectedPixelRangeX * pixelWidth + pixelWidth, selectedPixelRangeY * pixelHeight,
								selectedPixelRangeX * pixelWidth + pixelWidth,
								(selectedPixelRangeY * pixelHeight) + 10);
					}
				}
				gc.setXORMode(false);
	...
	private void drawLine(String id, Layer layer, int x1, int y1, int x2, int y2, ImagingWidgetConfiguration conf) {
		...
		if (x1 == x2 && y1 == y2) {
			...
			y = y1;
				 ...
				 * if (x1 == x2 && y1 != y2) { if (y1 < y2) { x = x1; y = y1; for (y = y1; y <
				 * y2; y += 1) { setPixel(id, layer, x, y, conf); } } else { x = x1; y = y2; for
				 * (y = y2; y < y1; y -= 1) { setPixel(id, layer, x, y, conf); } }
				 * 
				 * } else if (y1 == y2 && x1 != x2) {
				 * 
				 * if (x1 < x2) { x = x1; y = y1; for (x = x1; x < x2; x += 1) { setPixel(id,
				 * layer, x, y, conf); } } else { x = x2; y = y1; for (x = x2; x < x1; x -= 1) {
				 * setPixel(id, layer, x, y, conf); } } } else
				 */ {
			dx = x2 - x1;
			dy = y2 - y1;
			xstep = 1;
