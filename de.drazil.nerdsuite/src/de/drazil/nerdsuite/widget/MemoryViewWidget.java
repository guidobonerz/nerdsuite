...
public class MemoryViewWidget extends Canvas implements PaintListener {
	...
	private void prepareImage(int width, int height) {
		...
		/*
		 * gc.drawString("$0000-$1fff", 0, 210); gc.drawString("$2000-$3fff", 0, 440);
		 * gc.drawString("$4000-$5fff", 0, 670); gc.drawString("$6000-$7fff", 0, 900);
		 * gc.drawString("$8000-$9fff", 330, 210); gc.drawString("$a000-$bfff", 330,
		 * 440); gc.drawString("$c000-$dfff", 330, 670); gc.drawString("$e000-$ffff",
		 * 330, 900);
		 */
		gc.dispose();
	}
