...
public class AbstractStreamView {
	...
	protected void handleObjectOLD(RunObject runObject) {
	...
	protected void handleObject(RunObject runObject) {
		...
		String serviceUrl = String.format("http://%s/v1/runners:run_prg",
				System.getProperty(Constants.U64_HOST_ADDRESS));
		HttpClient client = HttpClient.newHttpClient();
		HttpRequest request = HttpRequest.newBuilder().uri(URI.create(serviceUrl))
				.POST(HttpRequest.BodyPublishers.ofByteArray(runObject.getPayload())).build();
		try {
