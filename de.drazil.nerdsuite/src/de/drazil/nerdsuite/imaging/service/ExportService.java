...
public class ExportService extends IOBaseService {
	...
	public void doExportGraphic(Map<String, Object> config) {
		 ...
		 * metadata.getViewerConfig().get(ProjectMetaData.PAINTER_CONFIG);
		 * 
		 * byte[] content = new byte[repository.getSize() * conf.getTileSize() /
		 * metadata.getStorageEntity()]; try { convert(content, 0, repository,
		 * ConversionMode.toBitplane); BinaryFileHandler.write(new File(fileName,
		 * "exported_font"), content, 0, content.length, true);
		 * 
